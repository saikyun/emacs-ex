...
(defcustom inf-clojure-load-form-cljs "(cljs.core/load-file \"%s\")"
...
(message "type %s" (inf-clojure--detect-repl-type proc))
;;    (message "wat %s" (inf-clojure--send-string proc (format (inf-clojure-load-form proc) file-name)))
    (when switch-to-repl
...
(defun inf-clojure--eq-response-p (proc form res)
  "Return true if PROC's response after evaluating FORM is equal to `res`."
  (inf-clojure--process-response-match-p
   (lambda (string)
     (equal res (string-trim string)))
   proc form))
